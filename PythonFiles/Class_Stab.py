...
class Stab(Part, Abstract):

    """ defines the stabilizer of the foil """

    def __init__(self):
        super(Stab, self).__init__()
        self._apex = (self._O + self._P + self.apex[0], self.apex[1], self.apex[2])
        self.dihedral_f = Foil.dbw
        self.dihedral_scale = 1.0
        self.dihedral_weight = 3.0
        self.sweep_f = Foil.sbw
        self.sweep_scale = 1.0
        self.sweep_weight = 1.0
        self.twist_s = Foil.tbw
        self.twist_scale = 1.0
        self.twist_weight = 1.0
        self.aoa = Foil.aoabw
        self.taper_ratio = (Foil.T/Foil.L)
        self.chord_f = -1.0 * self.taper_ratio + 1.0
        self.chord_scale = 1.0
        self.segment_num = int(Foil.numsegbw)
        ...
        list_points_dihedral = self.curve1cp(((0.0, 0.0, 0.0), (0.75, 0, 0), (1, 0.75 * self.dihedral_scale, 0.0), (1.0, 1 * self.dihedral_scale, 0.0)), 2, (1, 1, 2, 3, 3), (1, 10, self.dihedral_weight, 1), self.segment_num)
        ...
        list_points_sweep = self.curve1cp(((0.0, 0.0, 0.0), (0.7, 0.5 * self.sweep_scale, 0), (1, 1 * self.sweep_scale, 0.0)), 2, (1, 1, 2, 2), (1, self.sweep_weight, 1), self.segment_num)
        self.Lpoints_sweep = []
    ...
    def dihedral_function(self, epsilon=0.0):
        """ Dihedral angle evolution for the stab """
        eps_array = []
    ...
    def twist_function(self, epsilon=0.0):
        """ Twist angle evolution for the stab """
        eps_array = []
        for i in range(0, self.segment_num + 1):
            list.append(eps_array, float(i) / self.segment_num)
    ...
    def sweep_function(self, epsilon=0.0):
        """ Sweep angle evolution for the stab """
        eps_array = []
        for i in range(0, self.segment_num + 1):
            list.append(eps_array, float(i) / self.segment_num)
    ...
    def chord_function(self, epsilon=0.0):
        """ Chord length evolution for the stab """
        eps_array = []
        for i in range(0, self.segment_num + 1):
            list.append(eps_array, float(i) / self.segment_num)
    ...
    def airfoil_function(self, epsilon, LEPoint, ChordFunct, ChordFactor, DihedralFunct, TwistFunct):
        ...
        Airf, Chrd, ChordLength = primitives.Airfoil.AddAirfoilFromSeligFile(Af, Foil.stab_section)
        ...
        # imports the .dat file of the section to be used for the stab
        return Airf, Chrd

    def generate_wing(self):
        """ Generates the stab """
        stab_wing = liftingsurface.LiftingSurface(self.side, self._apex, self.sweep_function,
        ...
        ChordFactor = self._L / (self._M)  # of the stab's total length
        ScaleFactor = self._L / ChordFactor
        id = stab_wing.GenerateLiftingSurface(ChordFactor, ScaleFactor)[0]
        # creation of the stabilizer from the imported section
        return id
