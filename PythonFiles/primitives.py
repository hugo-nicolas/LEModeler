...
class Airfoil:
    ...
    def _TransformAirfoil(self, C, Part_type="", axe=0):
        ...
        if Part_type == "fuselage":
            ...
            rs.RotateObject(C, (1*self.ChordLength, 0, 0), self.Twist, rs.VectorCreate((0, 0, 0), (0, 1, 0)))
            rs.RotateObject(Chrd, (1*self.ChordLength, 0, 0), self.Twist, rs.VectorCreate((0, 0, 0), (0, 1, 0)))
        # Dihedral
        ...
        if axe == 1:
            rs.RotateObject(C, (0, 0, 0), -90, rs.VectorCreate((0, 0, 0), (0, 0, 1)))
            rs.RotateObject(Chrd, (0, 0, 0), -90, rs.VectorCreate((0, 0, 0), (0, 0, 1)))
        return C, Chrd, self.ChordLength
    ...
    def AddNACA4(self, MaxCamberPercChord, MaxCamberLocTenthChord, MaxThicknessPercChord, Smoothing=1):
        ...
        x, z, xu, zu, xl, zl, RLE = self._NACA4digitPnts(MaxCamberPercChord,
                                                         MaxCamberLocTenthChord, MaxThicknessPercChord)
    ...
    def AddNACA5(self, DesignLiftCoefficient, MaxCamberLocFracChord, MaxThicknessPercChord, Smoothing=1):
        ...
        x, z, xu, zu, xl, zl, RLE = self._NACA5digitPnts(DesignLiftCoefficient,
                                                         MaxCamberLocFracChord, MaxThicknessPercChord)
        C = self._fitAirfoiltoPoints(x, z)
        if 'Smoothing' in locals():
            self.SmoothingIterations = Smoothing
        C, Chrd = self._TransformAirfoil(C)
    ...
    def AddAirfoilFromSeligFile(self, AirfoilSeligName, Part_type="", axe = 0, Smoothing=1):
        ...
        if 'Smoothing' in locals():
            self.SmoothingIterations = Smoothing
        C, Chrd, ChordLength = self._TransformAirfoil(C, Part_type, axe)
        return C, Chrd, ChordLength
